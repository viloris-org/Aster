...
Aster 是一个 Rust 游戏引擎，配有原生编辑器——制作场景、调整物理、编写脚本，实时看到结果。
...
- **场景编辑器**——通过可视化界面放置对象、调整变换、添加组件。无需手写 JSON。
- **实时播放模式**——点击 Play，物理和脚本开始运行；点击 Stop，零清理。编辑场景从不被修改。
- **AI 辅助编辑**——用自然语言描述需求，Agent 在沙箱审查工作流中规划并执行场景变更。
- **资源管线**——将 glTF/PNG 拖入项目面板。文件监听自动触发导入，热重载实时推送更新。
- **可插拔渲染**——无需修改引擎代码即可切换后端。自带 WGPU；Vulkan 开发中。
- **无头运行时**——同一引擎可用于服务器、CI 测试或自动化构建，无需窗口。
- **无不安全代码**——每个 crate 使用 `#![forbid(unsafe_code)]`。
...
│   ├── engine-editor-ui/    # egui 面板、控件、视口渲染
...
| Profile | 包含内容 |
|---|---|
| `editor` | 完整编辑器，含 egui 面板、wgpu 视口、Agent 工具 |
